...
export class MMSDepartmentResponseDto {
    @ApiProperty({ description: '부서 ID', example: '67d0f0a79af04fc1b2f65ab1' })
    _id: string;

    @ApiProperty({ description: '부서명', example: '루미르 주식회사' })
    department_name: string;

    @ApiProperty({ description: '부서 코드', example: '루미르' })
    department_code: string;

    @ApiProperty({ description: '관리자 ID', example: null })
    manager_id: string | null;

    @ApiProperty({ description: '상위 부서 ID', example: null })
    ...
    @ApiProperty({ description: '하위 부서 목록', type: [MMSDepartmentResponseDto] })
